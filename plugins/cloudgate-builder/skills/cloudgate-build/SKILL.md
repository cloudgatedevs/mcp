...
- Tools surface as `mcp__cloudgate__*` once connected. SQLite database tools are on
  **both** `/mcp/workflow` and `/mcp/data`; this plugin connects `/mcp/workflow` only.
  The legacy Data Tables feature was retired (replaced by Worksheets) — there are no
